	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`dianyingleixing`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
					...
					return
																																								...
																																								if(this.ruleForm.id){
					await this.$api.update(`dianyingleixing`, this.ruleForm);
				}else{
					await this.$api.add(`dianyingleixing`, this.ruleForm);
				}
